...
resource "azurerm_virtual_machine" "reform-nonprod" {
  ...
  storage_data_disk {
    name          = "${element(data.template_file.server_name.*.rendered, count.index)}-DATA00"
    vhd_uri       = "https://${var.storage_account}.blob.core.windows.net/vhds/${element(data.template_file.server_name.*.rendered, count.index)}-DATA00.vhd"
    create_option = "Empty"
    caching       = "ReadWrite"
    lun           = 0
